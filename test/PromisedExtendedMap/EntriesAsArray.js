...
describe('EntriesAsArray', () => {
  it('Should call PromisedMap entries with no parameters', () => {
    const entriesAsArraySpy = sandbox.spy(PromisedMap.prototype, 'entries');
    return Reflect.construct(PromisedMap, [])
      .entriesAsArray()
      .should.be.fulfilled
      .then(() => {
        expect(entriesAsArraySpy.callCount).to.be.equal(1);
        expect(entriesAsArraySpy.getCall(0).args.length).to.be.equal(0);
